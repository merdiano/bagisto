...
@push('scripts')
    ...
    <script type="text/x-template" id="variant-item-template">
        <tr>
            <td>
                <input type="hidden" :name="[variantInputName + '[vendor_id]']" value="{{ $sellerProduct->marketplace_seller_id }}"/>
            ...
            <td class="actions">
                <a :href="['{{ route('marketplace.account.products.index') }}/edit/' + variant.id]"><i class="icon pencil-lg-icon"></i></a>
            ...
            methods: {
                ...
                sourceInventoryQty (inventorySourceId) {
                    var inventories = this.variant.inventories.filter(function(inventory) {
                        return inventorySourceId === inventory.inventory_source_id && inventory.vendor_id == "{{ $sellerProduct->marketplace_seller_id }}";
                    })
